...
// CODE HERE
const sayHelloButtonRemove = () => {
    sayHelloButton.style.backgroundColor = '#EFEFEF'
    sayHelloButton.style.color = 'black'
}

sayHelloButton.addEventListener('mouseout', sayHelloButtonRemove)

// sayHelloButton.addEventListener('mouseout', sayHelloButtonChange)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/Bobtoddwilliams').then((res) => {
...
const addQuery = () => {
    axios.get('http://localhost:3000/query-test/?bobtoddwilliams').then((res) => {
        console.log(res.data)
    }).catch(err => {
        console.log(err) 
    })
}
document.querySelector('#query-button').addEventListener('click',addQuery)
////////////////
//INTERMEDIATE//
////////////////
...
// food.Form.addEventListener('submit', createFood)